...
const BackTestingSession = () => {
  ...
  const fetchSessions = async () => {
    try {
      ...
      const response = await fetch("http://localhost:5001/api/users/sessions-backtest", {
        method: "GET",
        headers: {
      ...
      const sortedSessions = data.sort((a, b) => a.nom_actif.localeCompare(b.nom_actif));
...
const handleAddSession = async () => {
  ...
  try {
    ...
    const response = await fetch("http://localhost:5001/api/sessions/addSession", {
      method: "POST",
      headers: {
        Authorization: `Bearer ${token}`,
        "Content-Type": "application/json",
      ...
      body: JSON.stringify({
        ...newSession,
        date_debut: newSession.date_debut || new Date().toISOString().split("T")[0], // Utiliser la date actuelle si vide
      }),
    });
    ...
    await fetchSessions();
    setNewSession({ titre: "", objet: "", date_debut: new Date().toISOString().split("T")[0], capital: "", actif_id: "" });
...
const handleDeleteSession = async (sessionId) => {
  if (!window.confirm("⚠️ Êtes-vous sûr de vouloir supprimer cette session ? Cette action est irréversible.")) {
    return;
  }
  ...
  try {
    ...
    const response = await fetch(`http://localhost:5001/api/sessions/deleteSession/${sessionId}`, {
      method: "DELETE",
      headers: {
        Authorization: `Bearer ${token}`,
        "Content-Type": "application/json",
  ...
  const fetchObservations = async (sessionId) => {
    try {
      console.log(`📡 Récupération des observations pour la session ID: ${sessionId}...`);

      const response = await fetch(`http://localhost:5001/api/observations/observations/${sessionId}`);
      
      if (!response.ok) throw new Error("⚠️ Erreur lors de la récupération des observations");

      const data = await response.json();
      console.log(`✅ Observations reçues pour la session ${sessionId} :`, data);
      ...
      console.error(`❌ Erreur de récupération des observations pour la session ${sessionId} :`, error);
    }
  };
  ...
  const handleDeleteObservation = async (sessionId, observationId) => {
    ...
    try {
      console.log(`🗑️ Suppression de l'observation ID: ${observationId} pour la session ${sessionId}...`);
      ...
      if (!response.ok) throw new Error("⚠️ Erreur lors de la suppression de l'observation");

      setTimeout(() => {
        setObservations((prev) => ({
          ...prev,
          [sessionId]: prev[sessionId].filter(obs => obs.id !== observationId),
        }));
      ...
      console.error(`❌ Erreur lors de la suppression de l'observation ${observationId} :`, error);
      setLoadingObservations((prev) => ({
        ...prev,
        [sessionId]: false,
      }));
  ...
  useEffect(() => {
    if (selectedSession) {
        console.log("🔍 Valeur originale de `capital` dans `selectedSession` :", selectedSession.capital, "Type :", typeof selectedSession.capital);
        ...
        console.log("💰 Capital après conversion :", convertedCapital, "Type :", typeof convertedCapital);
    }
}, [selectedSession]);



  if (loading) return <p className="text-center text-gray-500 mt-10">Chargement des sessions de backtest...</p>;
    ...
    <div className="flex flex-col w-full mb-40 ">
      {viewMode ? (
        ...
        <div className="w-auto h-full">
          ...
          <div className="w-[98%] ">
            {viewMode === "addTransaction" && <AddTransaction sessionBacktestId={selectedSession.id} sessionDetails={selectedSession} actifId={selectedSession.actif_id} capital2={capital2}/>}

            {viewMode === "visualisation" && (
              <Visualisation sessionBacktestId={selectedSession.id} capital={capital2} />
            )}
          </div>
        ...
        <div className="w-full">
          ...
          {showAddSession && (
            <div className="w-[50%] bg-gray-200 dark:bg-gray-900 p-4 rounded-lg mb-10">
              <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-4">Nouvelle session</h3>
              ...
              <input
                ...
                className="w-full p-2 mb-4 border font-extralight text-xs rounded text-black bg-gray-300"
              />
              <input
                type="text"
                placeholder="Objet"
                value={newSession.objet}
                onChange={(e) => setNewSession({ ...newSession, objet: e.target.value })}
                className="w-full p-2 mb-4 border font-extralight text-xs rounded text-black bg-gray-300"
              />
              ...
              <div className="flex space-x-4">
                <input
                  ...
                  className="w-1/2 p-2 border font-extralight  text-xs rounded text-black bg-gray-300"
                />
                <select
                  value={newSession.actif_id}
                  onChange={(e) => setNewSession({ ...newSession, actif_id: e.target.value })}
                  className="w-1/2 p-2 border font-extralight  text-xs rounded text-black bg-gray-300"
                >
          ...
          {Object.keys(groupedSessions).map((actif) => (
            <div key={actif} className="mb-10">
              <div className="text-2xl font-bold text-gray-900 dark:text-white mb-4">{actif}</div>
              ...
              <div className="grid grid-cols-4 gap-6 w-[95%]">
                {groupedSessions[actif].map((session) => (
                  ...
                  >
                    {/* 📌 Contenu Session */}
                    <div className="p-2">
                      <h2 className="text-xl pb-2 font-semibold text-gray-900 dark:text-white">{session.titre}</h2>
                      <p className="text-gray-600 dark:text-gray-400 mt-2 font-extralight text-xs">{session.objet}</p>
                      <p className="text-gray-500 dark:text-gray-400 text-sm mt-4">
                        📅 Début : {new Date(session.date_debut).toLocaleDateString()}
                      </p>
                    </div>
